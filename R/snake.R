                   ...
                   methods=list(
                     ...
                     get_state=function(){
                       ...
                       for(nextdir in c("up","down","left","right")){
                         nextpos<-switch(nextdir,"up"=c(0,1),"down"=c(0,-1),"left"=c(-1,0),"right"=c(1,0))+body[1,]
                         danger_body<-as.numeric(any(do.call('c',lapply(1:nrow(body),function(x){all(body[x,]==nextpos)}))))
                         danger_wall<-as.numeric(any(nextpos%in%c(-1,nrow(board)+1,ncol(board)+1)))
                         danger[i]<-sum(c(danger_body,danger_wall))>0
                         i<-i+1
                       }

                       dirFruit<-as.numeric(c(body[1,1]<food[1],body[1,1]>food[1],body[1,2]<food[2],body[1,2]>food[2]))
                       ...
                       state_new<<-array(c(danger,dirFruit,movement),dim = c(1,12))
                     },
